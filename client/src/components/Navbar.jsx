  ...
  return <header className="sticky top-0 z-50 h-16 border-b border-indigo-100/70 bg-white/80 shadow-sm backdrop-blur-xl">
    <div className="mx-auto flex h-full max-w-6xl items-center justify-between px-4 sm:px-6">
      ...
      <div className="flex items-center gap-3 sm:gap-6">
        <nav className="hidden items-center gap-6 text-[14px] font-medium text-slate-600 sm:flex"><a href="#features">Features</a><a href="#how-it-works">How it Works</a><a href="#trust">Trust &amp; Evidence</a><a href="#history">History</a></nav>
        <span className="hidden h-4 w-px bg-indigo-100 sm:block" />
        <div className="relative">
          <button className="flex items-center gap-1.5 rounded-full border border-indigo-100 bg-white/90 px-3 py-1.5 text-[13px] font-medium text-slate-700 shadow-sm" onClick={() => setOpen(!open)}><Icon name="globe" size={15} className="text-indigo-500" />{language}<Icon name="chevron" size={14} className="text-slate-400" /></button>
          {open && <div className="absolute right-0 mt-2 w-36 rounded-xl border border-indigo-100 bg-white/95 py-1.5 text-[13px] shadow-xl shadow-indigo-500/10">{languages.map((item) => <button className={`flex w-full items-center justify-between px-3.5 py-1.5 text-left ${item === language ? 'font-semibold text-indigo-600' : 'text-slate-700'}`} key={item} onClick={() => { onLanguageChange(item); setOpen(false) }}>{item}{item === language && <Icon name="check" size={14} />}</button>)}</div>}
        </div>
